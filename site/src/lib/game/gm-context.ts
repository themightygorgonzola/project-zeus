...
function buildSystemPrompt(state: GameState, worldBrief: string): string {
	...
	parts.push(`    "companionPromoted": {"npcId": "existing-npc-id", "statBlock": {"hp": N, "maxHp": N, "ac": N, "speed": N, "cr": N, "abilities": {"str": N, ...}, "attacks": [{"name": "...", "toHit": N, "damage": "...", "damageType": "..."}], "savingThrows": [], "skills": [], "resistances": [], "immunities": [], "vulnerabilities": [], "traits": [], "actions": [], "legendaryActions": []}} or omit`);
	parts.push(`  },`);
...
export function buildStateExtractionPrompt(state: GameState): string {
	...
	parts.push(`    "encounterStarted": {"creatures": [{"id": "npc-<unique>", "name": "...", "role": "hostile", "locationId": "...", "disposition": -100, "description": "...", "tier": "weak|normal|tough|elite|boss"}]} or omit,`);
	parts.push(`    "encounterEnded": {"outcome": "victory|defeat|flee|negotiated"} or omit,`);
	parts.push(`    "companionPromoted": {"npcId": "existing-npc-id", "statBlock": {...}} or omit,`);
